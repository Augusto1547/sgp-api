...
public class UsuarioController {
  ...
  // Depois montar esse corpo no postaman(não estava funcionando)
  @PostMapping // @PostMapping caso quizesse usar post para tudo("/cadastrar")
  public ResponseEntity<Usuario> castrar(@RequestBody Usuario usuario) {
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(usuarioService.salvarUsuario(usuario));
  }

  @GetMapping // Aqui seria também @PostMapping("/listar")
  ...
  public ResponseEntity<Usuario> buscarPeloId(@PathVariable Long id) {
    ...
    if (usuario == null) { // Objects.isNull(usuario) jeito mais "chique"
